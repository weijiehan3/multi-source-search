...
Host-provided web search is orchestrated by the agent and not called from this script.
...
from portable_skill_runtime import find_search_credentials_file, load_env_chain
...
load_env_chain(Path(__file__).resolve())
...
def _find_credentials() -> str | None:
    ...
    cred_path = find_search_credentials_file(Path(__file__).resolve())
