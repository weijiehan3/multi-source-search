...
Notes
- This is NOT an MCP server. It's a script meant to be called by local skills via exec.
...
from portable_skill_runtime import default_workspace_root, load_env_chain

load_env_chain(pathlib.Path(__file__).resolve())

WORKSPACE = default_workspace_root(pathlib.Path(__file__).resolve())
...
def _http_json(method: str, url: str, *, headers: dict[str, str] | None = None, payload: dict | None = None, timeout: int = 60) -> dict:
    data = None
    hdrs = {"Accept": "application/json", "User-Agent": "portable-skill-mineru"}
    if headers:
        hdrs.update(headers)
...
def _http_bytes(url: str, *, headers: dict[str, str] | None = None, timeout: int = 180) -> bytes:
    hdrs = {"User-Agent": "portable-skill-mineru"}
    if headers:
        hdrs.update(headers)
